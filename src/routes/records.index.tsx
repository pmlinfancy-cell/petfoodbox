        ...
        {orders.map(o => {
          ...
          return (
            <Link key={o.id} to="/records/$orderId" params={{ orderId: o.id }}
              className="card-soft p-4 flex items-center gap-3 cursor-pointer">
              {cat
                ? <CatAvatar emoji={cat.emoji} color={cat.color} size={52} />
                : <div className="w-[52px] h-[52px] rounded-full bg-muted flex items-center justify-center text-2xl">❓</div>}
